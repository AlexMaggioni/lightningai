...
class MultiHeadedAttention(nn.Module):
    ...
    def get_attention_weights(self, queries, keys, mask=None):
        ...
        result = (queries @ keys.transpose(-2, -1)) / math.sqrt(self.head_size)

        # Apply the mask
        if mask is not None:
            mask = mask.unsqueeze(1).unsqueeze(2)
            result = result.masked_fill(mask == 0, float("-1e20"))
    ...
    def split_heads(self, tensor):
        ...
        new_dim = tensor.size(-1) // self.num_heads

        # Reshape and transpose the tensor
        tensor = tensor.view(tensor.size(0), tensor.size(1), self.num_heads, new_dim)
    ...
    def merge_heads(self, tensor):
        ...
        # Reshape and transpose the tensor
        tensor = tensor.transpose(1, 2).contiguous()
        tensor = tensor.view(tensor.size(0), tensor.size(1), new_dim)
...
class PreNormAttentionBlock(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        attention_inputs = self.layer_norm_1(x)
        atention_outputs = self.attn(attention_inputs, mask)

        new_x = x + atention_outputs

        # Then apply the feed-forward network
        layer_norm_outputs = self.layer_norm_2(new_x)
        linear_outputs = self.linear(layer_norm_outputs)
        outputs = new_x + linear_outputs
...
class Transformer(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # Take the cls token representation (first token of the sequence)
        cls_output = x[:, 0, :]

        # Pass the CLS token representation through the MLP head
